...
- **Invoice PDF**: tiap pembayaran lunas di riwayat pembayaran punya tombol ⬇ PDF (`/api/billing/orders/<id>/invoice.pdf`, hanya milik user sendiri). Dibuat server tanpa dependensi (`invoice.py`). Data penjual diisi `MYFLIPBOOK_INVOICE_SELLER` (baris dipisah `|`). Pembayaran Lemon Squeezy menghasilkan *Receipt* (faktur pajak resmi dari Lemon Squeezy), pembayaran simulasi diberi tanda TEST.
